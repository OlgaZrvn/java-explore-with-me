...
public class EventServiceImpl implements EventService {
    ...
    private void setConfirmedRequests(List<Event> events) {
        ...
        events.forEach(event ->
                event.setConfirmedRequests(Math.toIntExact(confirmedRequestsMap.getOrDefault(event.getId(), Long.valueOf(0)))));
    }
    ...
    @Override
    public EventFullDto updateEventByIdAdmin(Integer eventId, UpdateEventAdminRequest dto) {
    ...
    private void updateEventAdmin(Event event, UpdateEventAdminRequest eventDto) {
        ...
        if (eventDto.getStateAction() != null) {
            if (event.getState().equals(EventState.PENDING)) {
                ...
                throw new ValidationException("Состояние неверное: " + event.getState());
            }
        }
    ...
    private void updateEventCommonFields(Event event, UpdateEventAdminRequest eventDto) {
        ...
        if (eventDto.getCategory() != null) {
            Category category = categoryRepository.findById(eventDto.getCategory())
                    .orElseThrow(() -> new NotFoundException("Нет категории с id=" + eventDto.getCategory()));
    ...
    @Override
    @Transactional
    public EventFullDto updateEventByUserId(Integer userId, Integer eventId, UpdateEventAdminRequest dto) {
        Event event = eventRepository.findByIdAndInitiatorId(eventId, userId).orElseThrow(
                () -> new NotFoundException("Событие не найдено"));

        if (event.getState() == EventState.PUBLISHED || event.getEventDate().isBefore(LocalDateTime.now().plusHours(2))) {
            throw new ValidationException("Статус CANCELED или PENDING");
        }
    ...
    public EventRequestStatusUpdateResult updateEventRequestStatus(Integer userId,
        ...
        if (status == CONFIRMED || status == REJECTED) {
            ...
            if ((numberOfParticipants >= participantLimit) || (participantLimit == 0)) {
                throw new ValidationException("В событии уже максимальное кол-во участников");
            }
